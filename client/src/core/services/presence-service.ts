...
export class PresenceService {
  ...
  public createHubConnection(user: User) {
    ...
    this.hubConnection.on('NewMessageReceived', (message: Message) => {
      this.toast.info(message.senderDisplayName + $localize`:@@newMessageReceived:${message.senderDisplayName}:name: has sent you a new message`, 10000, message.senderImageUrl, `/members/${message.senderId}/messages`);
    });
  }
